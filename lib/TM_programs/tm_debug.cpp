...
int _tm_exec_state_wait_receive(uint8_t src)
{
    ...
    for (;;)
    {
        tm_sys.yield();
        // Time out
        if (millis() - start_time >= TM_TRANSFLOW_RECEIVE_TIMEOUT)
            return TM_TRANSFLOW_RECEIVE_TIMEOUT_ERR;
        ...
        if (ret != HU_PROT_RECEIVE_LISTENING && ret != HU_PROT_RECEIVE_IGNORE)
            break;
    ...
    if (ret != HU_PROT_RECEIVE_RECEIVED)
        return TM_TRANSFLOW_RECEIVE_CORRUPT_ERR;
...
int tm_exec_packet_send_full_open(String full_command)
{
    ...
    for (;;)
    {
        ...
        case state_retrans:
        {
            if (retry >= TM_TRANSFLOW_RECEIVE_RETRIES)
                return 1;
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            case 'a': // Address select
            {
                ...
                addr = arg.toInt();
                // Limit size
                if (addr >= sizeof(hu_packet_s))
                    addr = sizeof(hu_packet_s) - 1;
