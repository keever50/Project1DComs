...
int hu_protocol_transmit(RH_ASK *driver, hu_packet_t *packet)
{
    ...
    // Transmit //
    if (!driver->send(hu_protocol_buffer, HU_PROTOCOL_START_LENGTH + packet->length + 1 + offset))
        return 1; // Include LRC
    if (!driver->waitPacketSent(HU_PROTOCOL_TIMEOUT))
        return 2;
...
hu_prot_receive_err_t hu_protocol_receive(RH_ASK *driver, hu_packet_t *packet)
{
    ...
    if (!driver->recv(hu_protocol_buffer, &len))
        return HU_PROT_RECEIVE_LISTENING;
...
uint8_t hu_protocol_encode_address(const char *str)
{
    ...
    // Class
    if (strncmp(str, "2A", 2) == 0)
        classBits = 0b00;
    else if (strncmp(str, "2B", 2) == 0)
        classBits = 0b01;
    else if (strncmp(str, "2C", 2) == 0)
        classBits = 0b10;
    else if (strncmp(str, "2D", 2) == 0)
        classBits = 0b11;
    else
        return 0; // Invalid class

    // Group
    if (strncmp(str + 2, "G1", 2) == 0)
        groupBits = 0b001;
    else if (strncmp(str + 2, "G2", 2) == 0)
        groupBits = 0b010;
    else if (strncmp(str + 2, "G3", 2) == 0)
        groupBits = 0b011;
    else if (strncmp(str + 2, "G4", 2) == 0)
        groupBits = 0b100;
    else if (strncmp(str + 2, "G5", 2) == 0)
        groupBits = 0b101;
    else if (strncmp(str + 2, "G6", 2) == 0)
        groupBits = 0b110;
    else if (strncmp(str + 2, "G7", 2) == 0)
        groupBits = 0b111;
    else
        return 0; // Invalid group

    // Module
    if (strncmp(str + 4, "MM", 2) == 0)
        moduleBits = 0b000;
    else if (strncmp(str + 4, "CM", 2) == 0)
        moduleBits = 0b001;
    else if (strncmp(str + 4, "NM", 2) == 0)
        moduleBits = 0b010;
    else if (strncmp(str + 4, "CO", 2) == 0)
        moduleBits = 0b011;
    else if (strncmp(str + 4, "TM", 2) == 0)
        moduleBits = 0b100;
    else
...
{ // check function of het omzetten goed is gegaan
